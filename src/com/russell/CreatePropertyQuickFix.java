...
import com.russell.psi.RussellElementFactory;
...
class CreatePropertyQuickFix extends BaseIntentionAction {
      ...
      public void run() {
        ...
        if (virtualFiles.size() == 1) {
          ...
          final FileChooserDescriptor descriptor =
              FileChooserDescriptorFactory.createSingleFileDescriptor(RussellFileType.INSTANCE);
  ...
  private void createProperty(final Project project, final VirtualFile file) {
    new WriteCommandAction.Simple(project) {
      @Override
      public void run() {
        RussellFile RussellFile = (RussellFile) PsiManager.getInstance(project).findFile(file);
        ASTNode lastChildNode = RussellFile.getNode().getLastChildNode();
        // TODO: Add another check for CRLF
        if (lastChildNode != null/* && !lastChildNode.getElementType().equals(RussellTypes.CRLF)*/) {
          RussellFile.getNode().addChild(RussellElementFactory.createCRLF(project).getNode());
        }
        // IMPORTANT: change spaces to escaped spaces or the new node will only have the first word for the key
        RussellProperty property = RussellElementFactory.createProperty(project, key.replaceAll(" ", "\\\\ "), "");
        RussellFile.getNode().addChild(property.getNode());
